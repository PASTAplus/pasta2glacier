...
daiquiri.setup(
    level=logging.INFO,
    outputs=("stderr", daiquiri.output.File(filename=f"{log_file}")),
)
...
@click.option(
    "--lockfile", default="/tmp/glacier.lock", type=str, help=help_lock
)
def main(
    vault: str,
    data_path: str,
    ...
    try:
        ...
        for dir_name in data_directories(data_path):
            ...
            logger.debug(f"Directory {cnt} of {dirs}: {dir_name}")
            cnt += 1

            if (
                not gdb.package_exists(dir_name)
                and dir_name not in ignore_pids
            ):
                ...
                if os.path.exists(archive) and not force:
                    msg = (
                        f"Archive file {archive} exists, will use instead "
                        "of creating new archive"
                    )
                    logger.info(msg)
                else:
                ...
                if not dryrun:
                    ...
                    try:
                        ...
                        if archive_size < multipart_threshold:
                            response = glacier.do_upload(
                                archive=archive,
                                archive_description=archive_description,
                            )
                        else:
                            response = glacier.do_multipart_upload(
                                archive=archive,
                                archive_description=archive_description,
                                part_size=part_size,
                            )
                ...
                logger.debug(f"Skipping directory {dir_name}")
    except Exception as e:
        logger.ERROR(e)
    finally:
        lock.release()
